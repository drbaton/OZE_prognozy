...
def square(x, a, b):
    return (a * x * x) + (b * x)
##### fitting to function
(a, b), _ = opt.curve_fit(square, X, Y)
print(f'a={a}, b={b}')
##### rmse calculation
Y_pred = square(test_zmienne, a, b)
...
plt.scatter(train_zmienne, square(train_zmienne, a, b), s=2, c= 'red', marker='o')
plt.xlabel('i [W/m^2]')
plt.ylabel('E [MWh]')
plt.text(max(X)/3, 0, f'a = {a:.2e}, b = {b:.2e}', c= 'black')
plt.savefig('PV_square_fit.png', dpi=300)
plt.show()
...
plt.ylabel('E [MWh]')
plt.title(f'square rmse={rmse:.3f}, %error for max={(rmse/max_E)*100:.2f}%', c='black')
plt.legend(loc='upper center')
ax.set_ylim([0, 6])
plt.savefig('PV_square.png', dpi=300)
plt.show()
dane = np.stack((dt[train:], Y_pred), axis=1)
np.savetxt("PV_square.csv", dane, delimiter=";", header='hour;E_pred', fmt='%.3f')
##### sample forecast predict for next 5 hours
prognoza_i = np.array([0, 0, 100, 200, 500])
fcst_5h = square(prognoza_i, a, b)
print(fcst_5h)
